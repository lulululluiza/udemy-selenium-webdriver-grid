...
public class DesafioCadastroCompleto {
	...
	public void desafioCadastroCompleto() {
		...
		Assert.assertTrue(page.getResultadoCadastro("Cadastrado!"));
		//Assert.assertEquals("Nome: Fulano", driver.findElement(By.id("descNome")).getText());
		//ou
		Assert.assertTrue(page.getNomeCadastro("Fulano"));	
		Assert.assertEquals("Sobrenome: da Silva", page.getSobrenomeCadastro());
		Assert.assertEquals("Sexo: Feminino", page.getSexoCadastro());
		Assert.assertEquals("Comida: Frango", page.getComidaCadastro());
		Assert.assertEquals("Escolaridade: 2graucomp", page.getEscolaridadeCadastro());
		Assert.assertEquals("Esportes: Futebol Corrida Karate", page.getEsporteCadastro());
		Assert.assertEquals("Sugestoes: Nenhuma", page.getSugestoesCadastro());	
	}
}
